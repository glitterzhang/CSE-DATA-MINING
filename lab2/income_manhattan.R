...
income_ma<-matrix(0,nrow=nrow(income_train1),ncol=nrow(income_test1))
for(i in 1:nrow(income_train1))
{
  for(j in 1:nrow(income_test1))
  {  
    income_ma[i,j]<-abs(income_train1[i,1]-income_test1[j,1])+(as.character(income_train1[i,2])!=as.character(income_test1[j,2]))^2+
                           abs(income_train1[i,3]-income_test1[j,3])+abs(income_train1[i,4]-income_test1[j,4])+                                
                           ...
                           (as.character(income_train1[i,9])!=as.character(income_test1[j,9]))^2+ abs(income_train1[i,10]-income_test1[j,10])+
                           abs(income_train1[i,11]-income_test1[j,11])+abs(income_train1[i,12]-income_test1[j,12])+
...
##knn classification
k<-5
top_in<-matrix(0,nrow = ncol(income_ma),ncol = k)
index_in<-matrix(0,nrow = ncol(income_ma),ncol =k )
for(i in 1:ncol(income_ma))
{
  top_in[i,]<-head(sort(income_ma[,i]),k)
  index_in[i,]<-order(income_ma[,i])[1:k]
}
...
write.csv(income_knn,file="income_knn_manh.csv")
